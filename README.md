...
- **Reports** (JWT required)
  - `GET /reports/deleted-products` — Get percentage of deleted products
  - `GET /reports/existing-products` — Get percentage of non-deleted products
    - Query params:
      - `withPrice` (`true`/`false`): filter by presence of price
      - `startDate`, `endDate`: filter by creation date range (ISO format)
  - `GET /reports/stock-summary` — Get stock summary for products
...
- Use the JWT token for all `/reports` endpoints by setting the `Authorization` header.
